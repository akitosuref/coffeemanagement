...
import com.group13.coffeemanagement.model.Order;
import com.group13.coffeemanagement.model.OrderItemView;
...
//	totalPrice.setText(convertToVND(totalPriceLocal));
	...
	for (Order order : OrderBillDB.orders) {
		...
		if (foundFood != null) {
		    OrderItemView itemView = new OrderItemView(foundFood.getName(), order.getCount(), foundFood.getPrice(), foundFood.getId());
	...
	quantityCol.setOnEditCommit(event -> {
	    ...
	    OrderBillDB.orders.stream()
		    .filter(orderInfo -> orderInfo.getBillID() == chosenBill && orderInfo.getFoodID() == item.getFoodID()) // Use chosenBillID to identify the bill
		    .findFirst()
		    .ifPresent(orderInfo -> {
...
//        Category selectedCategory = categoryChoiceBox.getSelectionModel().getSelectedItem();
	...
	totalPrice.setText(convertToVND(totalPriceLocal));
	System.out.println("Added food ID: " + choseFood + " with quantity: " + quantity + " to table ID: " + choseTable);
    }
